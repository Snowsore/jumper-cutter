...
def render(inputFile, outputFile, threshold):
    ...
    # Sensetive
    mask = (data[:,0] > threshold) | (data[:,0] < -threshold)
    ...
    concatenate_videoclips(clips).write_videofile(outputFile, codec='mp4', ffmpeg_params=['-c:v', 'h264_nvenc', '-c:a', 'aac', '-vf', 'setpts=0.666*PTS', '-af', 'atempo=1.5', '-f', 'mp4'])
